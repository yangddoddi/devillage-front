...
export const PostView = () => {
  ...
  const selector = useSelector((state) => state.token.userId);
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setBookmarkLike(data.bookmarked);
  ...
  return (
    <>
      <div className={styles.container}>
        ...
        <div className={styles.replyContainer}>
          ...
          {reply.map((reply) => (
            <Reply
              ...
              setRender={setRender}
              setReplyDeleteModal={setReplyDeleteModal}
        ...
        <ReplyEditor postId={id} setRender={setRender} render={render} />
      </div>
      {reportModal && (
      ...
      {deleteModal && (
        <div className={styles.deleteModal}>
          ...
          <h1>게시글 삭제</h1>
          <p>정말로 삭제하시겠습니까?</p>
          <button onClick={clickPostDeleteBtn}>삭제</button>
        </div>
      )}
    </>
  );
