...
export const PostEditor = () => {
  ...
  const fileUpload = async (blob, callback) => {
    try {
      ...
      await axios
        ...
        .then((res) => {
          ...
          callback(res.data.remotePath, res.data.filename);
        }
        );
    } catch (err) {
      console.log(err);
    ...
    formData.append("file", blob);
    axios
      .post(`${SERVER}/files/${param.id}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
      ...
      .then((res) => {
        const url = res.data.remotePath;
        setFileIds((prev) => [...prev, url]);
        callback(url, "이미지 오류");
      })
      .catch((err) => {
        console.log(err);
